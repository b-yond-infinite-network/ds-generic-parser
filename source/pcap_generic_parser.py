...
def convert_pcap(input_folder_path: Path, output_folder_path: Path, file_name: str):
    in_path = input_folder_path.absolute()
    out_path = Path.joinpath(output_folder_path, file_name + '.json').absolute()
    return subprocess.run(f'cd "{in_path}"; tshark -r "{file_name}.pcap" -T json >"{out_path}"', capture_output=True, shell=True)


def read_parse_sip(pcap_folder_path, json_file):
    filename = pcap_folder_path / json_file
    with filename.open(encoding="utf8", errors="ignore") as f:
        return parse_sip(json.load(f, object_pairs_hook=custom_hook), json_file)


def read_parse_gtp(pcap_folder_path, json_file, gtp_causes):
    filename = pcap_folder_path / json_file
    with filename.open(encoding="utf8", errors="ignore") as f:
        return parse_gtp(json.load(f, object_pairs_hook=custom_hook), json_file, gtp_causes)


def read_parse_diameter(pcap_folder_path, json_file, diametercodes):
    filename = pcap_folder_path / json_file
    with filename.open(encoding="utf8", errors="ignore") as f:
        return parse_diameter_errors(json.load(f, object_pairs_hook=custom_hook), json_file, diametercodes)


def read_json_parallel(pcap_folder_path: Path, json_file):
    filename = pcap_folder_path / json_file
    with filename.open(encoding="utf8", errors="ignore") as f:
        return json.load(f, object_pairs_hook=custom_hook)


def tshark_aggregate_gtp_cause(pcap_folder_path: Path, file_name: str):
    values = []
    file_path = pcap_folder_path / file_name
...
def tshark_aggregate_diameter_result_code(pcap_folder_path: Path, file_name: str):
    values = []
    file_path = pcap_folder_path / file_name
